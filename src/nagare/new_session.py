...
class NewSessionForm(Vertical):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Input(placeholder="~/Projects/my-project", id="path-input")
        ...
        yield Static(
            "\n  [b]Enter[/b] Create   [b]Tab[/b] Accept suggestion / Next field   [b]Esc[/b] Cancel",
            id="form-hint",
        )
    ...
    def accept_suggestion(self) -> None:
        """Accept the top suggestion into the path input."""
        if self._suggestions:
            path_input = self.query_one("#path-input", Input)
            path_input.value = self._suggestions[0]
...
class NewSessionApp(App):
    ...
    def on_key(self, event) -> None:
        if event.key == "tab":
            ...
            # Only accept suggestion if there are suggestions and path input is focused
            if form._suggestions and self.focused and self.focused.id == "path-input":
                form.accept_suggestion()
                event.prevent_default()
                event.stop()
            # Otherwise let Tab do normal focus cycling
        elif event.key == "enter":
            focused = self.focused
            if focused and focused.id == "path-input":
                form = self.query_one("#new-session-form", NewSessionForm)
                if form._suggestions:
                    form.accept_suggestion()
                    event.prevent_default()
                    event.stop()
                    return
